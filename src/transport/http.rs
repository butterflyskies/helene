...
async fn run_sse_stream(
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        for &byte in chunk.iter() {
            if byte == b'\n' {
                ...
                if let Some(outcome) =
                    dispatch_line(&mut line_buf, &mut parser, signer, tx, &mut had_data).await
                {
                    return outcome;
                }
            } else if byte == b'\r' {
                ...
                if line_buf.len() >= max_line_bytes {
                    return SseOutcome::Error {
                        message: format!("SSE line exceeds {max_line_bytes} byte limit"),
                        had_data,
                    };
                }
                line_buf.push(byte);
    ...
    if !line_buf.is_empty() {
        let _ = dispatch_line(&mut line_buf, &mut parser, signer, tx, &mut had_data).await;
    }
